...
:ok = Cashier.Checkout.scan(co, "CF1")
:ok = Cashier.Checkout.scan(co, "SR1")
:ok = Cashier.Checkout.scan(co, "CF1")
:ok = Cashier.Checkout.scan(co, "CF1")
...
1. Create a module implementing `Cashier.PricingRule`:

```elixir
defmodule Cashier.PricingRules.PercentageOff do
  ...
  def calculate(items, product_code, opts) do
    rule_code = Keyword.fetch!(opts, :product_code)

    if product_code != rule_code do
      # Not our product — return full price
      quantity = Enum.count(items, &(&1 == product_code))
      Decimal.mult(Catalog.fetch!(product_code).price, quantity)
    else
      percentage = Keyword.fetch!(opts, :percentage)
      quantity = Enum.count(items, &(&1 == product_code))
      price = Catalog.fetch!(product_code).price

      price
      |> Decimal.mult(quantity)
      |> Decimal.mult(Decimal.sub(1, Decimal.div(percentage, 100)))
      |> Decimal.round(2)
    end
  end
end
...
2. Add it to your pricing rules list:

```elixir
pricing_rules = [
  {Cashier.PricingRules.PercentageOff, product_code: "GR1", percentage: 10},
  # ... other rules
]
```
